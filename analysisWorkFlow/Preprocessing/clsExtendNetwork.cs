...
{
    class clsExtendNetwork
    {
        public static void extent_Network(ref gProAnalyzer.GraphVariables.clsGraph graph, int currentN, int addNum)
        {
            
            //AddNum is the number of node are added
            //A full Graph (extended)
            gProAnalyzer.GraphVariables.clsGraph.strGraph saveNetwork = graph.Network[currentN];

            graph.Network[currentN] = new gProAnalyzer.GraphVariables.clsGraph.strGraph();

            graph.Network[currentN].header = saveNetwork.header;

            graph.Network[currentN].nNode = saveNetwork.nNode + addNum;
            ...
            graph.Network[currentN].nLink = saveNetwork.nLink + addNum;
