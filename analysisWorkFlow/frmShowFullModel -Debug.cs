...
using gProAnalyzer.GraphVariables;

namespace gProAnalyzer
{
    public partial class frmShowFullModel_Debug : Form
    {
        ...
        public frmShowFullModel_Debug(String inputModel, gProAnalyzer.GraphVariables.clsGraph graph_org, int currentN)
        {
            ...
            graph.Network[graph.untangleNet] = graph.Network[graph.orgNet];
            gProAnalyzer.Ultilities.extendGraph.full_extentNetwork(ref graph, graph.untangleNet, 0, 0);

            graph.Network[graph.orgNet] = graph_org.Network[currentN];

            //mnuLoadNetwork_Click(null, new ResponseReceivedEventArgs(), "");
            modelName = inputModel;
            mnuLoadNetwork_Click(this, EventArgs.Empty);
            //mnuAnalysisNetwork_Click(this, EventArgs.Empty);

            graph.Network[graph.orgNet] = graph.Network[graph.untangleNet];
            gProAnalyzer.Ultilities.extendGraph.full_extentNetwork(ref graph, graph.orgNet, 0, 0);
        }



        // Load the network for analysis and display the information
        public void mnuLoadNetwork_Click(object sender, EventArgs e1)
        {
            Initialize_All();

            //loadGraph.Load_Data(ref graph, graph.orgNet, modelName, true);
        ...
        //Run Analysis
        public void mnuAnalysisNetwork_Click(object sender, EventArgs e)
        {
        ...
        public void Display_Error(int currentN) //Display into GridView and Mark the gateways also
        {
        ...
        public void Display_SESE_Error(int currentN)
        {
            // Rule 4
            int id = 0;
            for (int i = 0; i < clsError.nError; i++)
        ...
        public void Draw_DomTree(int currentN, int workLoop, int workSESE, GoView goViewer, bool bRev)
        {
        ...
        public void Draw_Loop_SESE(int currentN, int workLoop, int workSESE, GoView goViewer)
        {
            int nNode = graph.Network[currentN].nNode;
            //////////////////////////////
            int nLoop = 0, nSESE = 0;
        ...
        public void Draw_Forest(int currentN, int workLoop, int workSESE, GoView goViewer, bool bRev)
        {
            goViewer.Document.Clear();
            GoDocument doc = goViewer.Document;
        ...
        public void tabGoView_SelectedIndexChanged(object sender, EventArgs e)
        {
        ...
        public int getFBLOCK_Index(int curSESE)
        {
            for (int i = 0; i < clsHWLS.FBLOCK.nFBlock; i++)
            {
        ...
        public void backgroundWorker1_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            Draw_Network(graph.finalNet, clsLoop.orgLoop, clsSESE.finalSESE, goViewE); // Entire
        ...
        public string isEntry_Exit(int node)
        {
            for (int i = 0; i < clsHWLS.FBLOCK.nFBlock; i++)
            {
